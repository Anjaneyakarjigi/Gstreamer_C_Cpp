...
int main(int argc, char *argv[])
{
	...
	g_object_set(G_OBJECT(udpsink), "port", port, "host", IpAddress, "sync", TRUE, "async", FALSE, "qos", TRUE, "qos-dscp", 46, NULL);
	...
	gst_bin_add_many(GST_BIN(pipeline), source, demux, parser, decoder, encoder, v_queue, mux, udpsink, pay, NULL);
	...
	if (gst_element_link_many(mux, pay , udpsink, NULL) != TRUE) {
		g_printerr("Many Elements could not be linked.\n");
		gst_object_unref(pipeline);
		return -1;
	}
