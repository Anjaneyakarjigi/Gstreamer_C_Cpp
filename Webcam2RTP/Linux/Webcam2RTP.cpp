...
int main(int argc, char *argv[])
{
	...
	GstElement *pipeline, *videosource, *videoconvert, *videoencoder, *videoqueue, *mux, *sink, *pay;
	GstBus *bus;
	GMainLoop *loop;
	GstMessage *msg;
	...
	pay = gst_element_factory_make("rtpmp2tpay", "payload");
	...
	/* Build the pipeline */
	gst_bin_add_many(GST_BIN(pipeline), videosource, videoconvert, videoencoder, videoqueue, mux , sink, pay,NULL);
	if (gst_element_link_many(videosource, videoconvert, videoencoder, videoqueue,mux , pay ,sink, NULL) != TRUE) {
	...
	g_print("Open Server at > rtp://@%s:%d\n",IpAddress , port);
	g_print("Running...\n");
